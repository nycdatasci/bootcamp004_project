...
d_tfidf = TfidfVectorizer(ngram_range=(1, 2), stop_words='english')
...
clf = pipeline.Pipeline([('union', FeatureUnion(transformer_list = [('cst',  cust_regression_vals()),
                                                                    ...
                                                                    ('txt12', pipeline.Pipeline([('s1', cust_txt_col(key='search_term')), ('tfidf12', d_tfidf), ('tsvd12', tsvd)])),
                                                                    ('txt22', pipeline.Pipeline([('s2', cust_txt_col(key='product_title')), ('tfidf22', d_tfidf), ('tsvd22', tsvd)])),
                                                                    ('txt32', pipeline.Pipeline([('s3', cust_txt_col(key='product_description')), ('tfidf32', d_tfidf), ('tsvd32', tsvd)])),
                                                                    ('txt42', pipeline.Pipeline([('s4', cust_txt_col(key='brand')), ('tfidf42', d_tfidf), ('tsvd42', tsvd)]))
                                                                    ],
...
# 0.00804813347366
# Best parameters found by grid search:
# {'rfr__max_features': 11, 'rfr__max_depth': 25}
# Best CV score:
# -0.46119246164
# 0.00883953110026
...
### Introduction
We found the Forest Cover type dataset in the UCI Machine Learning Repository that takes forestry data from four wilderness areas in Roosevelt National Forest in northern Colorado ([Click here](https://tourbuilder.withgoogle.com/tour/ahJzfmd3ZWItdG91cmJ1aWxkZXJyEQsSBFRvdXIYgICAk5uU5AoM) for a tour of the area). The observations are taken from 30m by 30m patches of forest that are classified as one of seven cover types:
...
The Kaggle training data was not a simple random sample of the entire dataset, but a stratified sample of the seven forest classes. The class distribution of the training data was equally distributed among the seven classes. 
</br>
...
<center>Click/drag on the plot to explore:</center>
...
The nnet R library allows the construction of feed-forward single-layer nerual networks. After scaling our variables and running a basic model, we decided to tune the nnet function based on the following parameters 
- Number of nodes 
...
*min_rows* is the minimum number of rows to assign to terminal nodes 

- Hyperparameter tuning gives an impvroment to **78.05%**.
...
Neural Network - Deep Learning    | No  | H2o package, 54-500-800-7, rate=0.02, rate-annealing=1e-05, rate-decay=1 |**0.67428**|
...
**Ensembles**                     |     |                                            |        | 
ensemble1 | No | basic voting of RF, GBM, ExtraTrees and XGBoost | 0.76 | 
ensemble2 | No | Feeding ExtraTrees to H2o XGB | 0.79182 | 
ensemble3 | No | Feeding ExtraTrees to H2o GBM then to XGB | 0.79182 | 

...
A 10-fold cross validated lasso-regularized logistic regression kaggle score = **0.59594**, 1411th place for 70-30 split kaggle score of **0.59526**, 1415th place for 85-15 split kaggle score of **0.59443** if we took the lowest 50 percintile of lambdas and calculated the mode of the predictions.

#### Elastic-net Regularization

Using both the ridge and lasso regularization terms concurrently produces an elastic-net regression. We ran this hybrid method with both a 70-30 split and 85-15 split in the training set. When running these models on the testing set, we got accuracies on Kaggle of **0.5959** (1,412th place) and **0.5952** (1,415th place), respectively.

We also tried an elastic-net regression using all 15,120 observations in the training set. However, since this did not use cross validation, it likely overfitted slightly, earning an accuracy score on the testing set of **0.5950** (1,417th place).
